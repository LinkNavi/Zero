  ...
  void Update(float deltaTime) override {
    for (auto entity : mEntities) {
      if (!ecs->HasComponent<Transform>(entity) ||
          !ecs->HasComponent<Rotator>(entity)) {
        continue;
      }
...
void keyCallback(GLFWwindow *window, int key, int scancode, int action,
  ...
  if (key == GLFW_KEY_SPACE && action == GLFW_PRESS) {
    ...
    std::cout << "Spawned cube! Total entities: "
              << state.ecs.GetSystem<RenderSystem>()->mEntities.size()
              << std::endl;
  }
}

Entity CreateCube(glm::vec3 position, glm::vec3 rotationSpeed,
...
GLFWwindow* window = glfwCreateWindow(1280, 720, "ZeroEngine", nullptr, nullptr);
if (!window) {
...
try {
  ...
  // Setup camera
  glm::mat4 view =
      glm::lookAt(glm::vec3(0.0f, 3.0f, 10.0f), glm::vec3(0.0f, 0.0f, 0.0f),
  ...
  for (int x = -1; x <= 1; x++) {
    for (int z = -1; z <= 1; z++) {
      ...
      glm::vec3 rotSpeed((x + 1) * 0.5f, (z + 1) * 0.5f, (x * z) * 0.3f);
      CreateCube(pos, rotSpeed, glm::vec3(0.8f));
    }
  }

  std::cout << "Created " << renderSystem->mEntities.size() << " cubes"
            << std::endl;
  std::cout << "\n==================================" << std::endl;
