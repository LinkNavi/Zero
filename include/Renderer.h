...
// OPTIMIZED: Double buffering is sufficient for iGPU
...
class VulkanRenderer {
  ...
  uint32_t windowWidth, windowHeight; // Actual window size (for UI)
...
public:
  bool init(uint32_t w, uint32_t h, const char *title) {
    ...
    glfwWindowHint(GLFW_SCALE_FRAMEBUFFER, GLFW_FALSE); // Disable auto scaling
    ...
    // Use whatever size we actually got
    int fbW, fbH;
    glfwGetFramebufferSize(window, &fbW, &fbH);
    width = static_cast<uint32_t>(fbW);
    height = static_cast<uint32_t>(fbH);

    std::cout << "  Requested: " << w << "x" << h << std::endl;
    std::cout << "  Got: " << width << "x" << height << std::endl;
    ...
    auto physRet =
        selector.set_surface(surface).set_minimum_version(1, 0).select();
    if (!physRet)
      return false;

    vkb::PhysicalDevice vkbPhysicalDevice = physRet.value();
    std::cout << "GPU: " << vkbPhysicalDevice.name << std::endl;
 ...
 void initImGui() {
    ...
    // Pass false to NOT install callbacks - we'll handle input ourselves
...
private:
  ...
  bool createDepthResources() {
    ...
    return vkCreateImageView(device, &viewInfo, nullptr, &depthImage.view) ==
           VK_SUCCESS;
  }

  bool createSwapchain() {
    // Query actual surface capabilities - Wayland/Hyprland may override size
    VkSurfaceCapabilitiesKHR surfaceCaps;
    vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physicalDevice, surface,
                                              &surfaceCaps);

    // Use the surface's current extent if it's defined
    ...
    width = std::max(surfaceCaps.minImageExtent.width,
                     std::min(surfaceCaps.maxImageExtent.width, width));
    height = std::max(surfaceCaps.minImageExtent.height,
                      std::min(surfaceCaps.maxImageExtent.height, height));

    std::cout << "  Swapchain size: " << width << "x" << height << std::endl;

    vkb::SwapchainBuilder swapchainBuilder{vkbDevice};
    auto swapRet = swapchainBuilder
                       .set_desired_format({VK_FORMAT_B8G8R8A8_UNORM,
                                            VK_COLOR_SPACE_SRGB_NONLINEAR_KHR})
                       // IMMEDIATE = no vsync, maximum FPS for benchmarking
  ...
  bool createRenderPass() {
    ...
    return vkCreateRenderPass(device, &rpInfo, nullptr, &renderPass) ==
           VK_SUCCESS;
  }
