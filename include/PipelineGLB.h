...
// Push constants for GLB models with material support
struct PushConstantsGLB {
    glm::mat4 mvp;
    glm::mat4 model;
    glm::vec3 lightDir;
    float padding1;
...
public:
    bool init(VkDevice dev, VkRenderPass renderPass, const std::string& vertPath, const std::string& fragPath) {
        ...
        if (vertCode.empty() || fragCode.empty()) {
            std::cerr << "Failed to load GLB shader files!" << std::endl;
            return false;
        }
        ...
        // Vertex input for GLB format: position(vec3) + normal(vec3) + texCoord(vec2)
        VkVertexInputBindingDescription bindingDescription{};
        bindingDescription.binding = 0;
        bindingDescription.stride = sizeof(float) * 8; // 3 + 3 + 2
        ...
        pushConstantRange.size = sizeof(PushConstantsGLB);
        ...
        if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS) {
            std::cerr << "Failed to create GLB pipeline layout!" << std::endl;
            return false;
        }
        ...
        if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline) != VK_SUCCESS) {
            std::cerr << "Failed to create GLB graphics pipeline!" << std::endl;
            return false;
        }
    ...
    void pushConstants(VkCommandBuffer cmd, const PushConstantsGLB& constants) {
