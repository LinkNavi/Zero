...
void Editor::drawUI(VkCommandBuffer cmd) {
    ...
    if (panels.viewport) {
        ...
        float vpW = centerW + (panels.hierarchy ? 0 : LEFT_W) + (panels.inspector ? 0 : RIGHT_W);
    ...
    if (panels.assets) {
        ...
        if (!panels.console) { assetsX = ox; assetsW = W - RIGHT_W; }
    ...
    // ---- Popups (float freely) ----
    if (panels.sceneSettings) drawSceneSettings();
    if (showFileDialog) drawFileDialog();

    // ---- Keyboard shortcuts ----
    if (io.KeyCtrl && ImGui::IsKeyPressed(ImGuiKey_N)) newScene();
    if (io.KeyCtrl && ImGui::IsKeyPressed(ImGuiKey_S)) saveScene();
    if (io.KeyCtrl && ImGui::IsKeyPressed(ImGuiKey_O)) openScene();
    if (io.KeyCtrl && ImGui::IsKeyPressed(ImGuiKey_D)) duplicateEntity();
    if (ImGui::IsKeyPressed(ImGuiKey_Delete) && !io.WantTextInput) deleteSelectedEntity();
    if (ImGui::IsKeyPressed(ImGuiKey_F5)) playScene();
    if (ImGui::IsKeyPressed(ImGuiKey_F6)) pauseScene();
    if (ImGui::IsKeyPressed(ImGuiKey_F7)) stopScene();
}
...
void Editor::drawMenuBar() {
    if (ImGui::BeginMenuBar()) {
        if (ImGui::BeginMenu("File")) {
            ...
            ImGui::Separator();
            if (ImGui::MenuItem("Exit")) glfwSetWindowShouldClose(renderer.getWindow(), GLFW_TRUE);
            ImGui::EndMenu();
        }
        if (ImGui::BeginMenu("Edit")) {
            if (ImGui::MenuItem("Duplicate", "Ctrl+D", false, selectedEntity != INVALID_ENTITY)) duplicateEntity();
            if (ImGui::MenuItem("Delete", "Del", false, selectedEntity != INVALID_ENTITY)) deleteSelectedEntity();
            ImGui::EndMenu();
        }
        if (ImGui::BeginMenu("Entity")) {
            ...
            if (ImGui::MenuItem("Create Cube")) {
                ...
                selectedEntity = e; unsavedChanges = true;
            }
...
void Editor::drawToolbar(float x, float y, float w, float h) {
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_WindowBorderSize, 0.0f);
    ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(0.118f, 0.122f, 0.149f, 1.0f));
    ...
    if (isPlaying)
        ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.173f, 0.506f, 0.278f, 1.0f));
    else
        ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.200f, 0.208f, 0.247f, 1.0f));

    if (ImGui::Button(isPlaying ? "Pause" : "Play", ImVec2(btnW, btnH))) {
        if (isPlaying) pauseScene(); else playScene();
    }
    ImGui::PopStyleColor();
    ImGui::SameLine(0, 6);
    ...
    ImGui::PushStyleColor(ImGuiCol_Button,
        canStop ? ImVec4(0.592f, 0.184f, 0.184f, 1.0f) : ImVec4(0.200f, 0.208f, 0.247f, 1.0f));
    if (ImGui::Button("Stop", ImVec2(btnW, btnH))) stopScene();
    ImGui::PopStyleColor();
    ...
    auto gizmoBtn = [&](const char* label, GizmoMode mode, float width) {
        ...
        if (sel) ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.286f, 0.494f, 0.882f, 0.4f));
        if (ImGui::Button(label, ImVec2(width, btnH))) gizmoMode = mode;
...
void Editor::drawHierarchyContent() {
    ...
    for (const auto& e : entities) {
        ...
        if (e.id == selectedEntity) flags |= ImGuiTreeNodeFlags_Selected;

        ImGui::TreeNodeEx((void*)(intptr_t)e.id, flags, "%s %s", icon, e.name.c_str());

        if (ImGui::IsItemClicked()) selectedEntity = e.id;
        ...
        if (ImGui::BeginPopupContextItem()) {
            ...
            ImGui::Separator();
            if (ImGui::MenuItem("Focus Camera")) engine.setEditorCameraTarget(e.position);
            ImGui::EndPopup();
        }
    }
    ...
    if (ImGui::BeginPopupContextWindow("##HCtx", ImGuiPopupFlags_NoOpenOverItems)) {
        if (ImGui::MenuItem("Create Empty")) {
            selectedEntity = engine.createEntity("Empty"); unsavedChanges = true;
        }
        if (ImGui::MenuItem("Create Cube")) {
            EntityID e = engine.createEntity("Cube");
            engine.setEntityModel(e, ResourcePath::models("cube.obj"));
...
static bool drawVec3(const char* label, glm::vec3& v) {
    ...
    ImGui::PushStyleColor(ImGuiCol_FrameBgHovered, ImVec4(0.60f, 0.20f, 0.20f, 0.60f));
    ImGui::SetNextItemWidth(w);
    if (ImGui::DragFloat("##X", &v.x, 0.05f, 0, 0, "%.2f")) changed = true;
    ImGui::PopStyleColor(2);
    ImGui::SameLine();

    // Y
    ImGui::PushStyleColor(ImGuiCol_FrameBg, ImVec4(0.15f, 0.50f, 0.15f, 0.50f));
    ImGui::PushStyleColor(ImGuiCol_FrameBgHovered, ImVec4(0.20f, 0.60f, 0.20f, 0.60f));
    ImGui::SetNextItemWidth(w);
    if (ImGui::DragFloat("##Y", &v.y, 0.05f, 0, 0, "%.2f")) changed = true;
    ImGui::PopStyleColor(2);
    ImGui::SameLine();

    // Z
    ImGui::PushStyleColor(ImGuiCol_FrameBg, ImVec4(0.15f, 0.15f, 0.50f, 0.50f));
    ImGui::PushStyleColor(ImGuiCol_FrameBgHovered, ImVec4(0.20f, 0.20f, 0.60f, 0.60f));
    ImGui::SetNextItemWidth(w);
    if (ImGui::DragFloat("##Z", &v.z, 0.05f, 0, 0, "%.2f")) changed = true;
    ImGui::PopStyleColor(2);
...
void Editor::drawInspectorContent() {
    ...
    if (ImGui::CollapsingHeader("Transform", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        if (drawVec3("Position", pos)) { engine.setEntityPosition(selectedEntity, pos); unsavedChanges = true; }
        if (drawVec3("Rotation", rot)) { engine.setEntityRotation(selectedEntity, rot); unsavedChanges = true; }
        if (drawVec3("Scale", scl))    { engine.setEntityScale(selectedEntity, scl); unsavedChanges = true; }
    }
    ...
    if (ImGui::CollapsingHeader("Model", ImGuiTreeNodeFlags_DefaultOpen)) {
        if (info.hasModel) {
            ...
            if (ImGui::Button("Remove Model", ImVec2(-1, 0))) {
                engine.removeEntityModel(selectedEntity); unsavedChanges = true;
            }
        } else {
            ...
            if (ImGui::Button("Load", ImVec2(-1, 0)) && strlen(mp) > 0) {
                if (engine.setEntityModel(selectedEntity, mp)) {
                    unsavedChanges = true; log("Loaded: " + std::string(mp));
                } else {
    ...
    if (info.isCamera) {
        if (ImGui::CollapsingHeader("Camera", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            if (ImGui::Checkbox("Active", &active)) {
                if (active) engine.setActiveCamera(selectedEntity);
            }
        }
    } else {
        ImGui::Spacing();
        if (ImGui::Button("+ Camera", ImVec2(-1, 0))) {
            engine.setEntityAsCamera(selectedEntity, false); unsavedChanges = true;
        }
    }
    ...
    if (ImGui::BeginDragDropTarget()) {
        if (const ImGuiPayload* p = ImGui::AcceptDragDropPayload("ASSET_MODEL")) {
            ...
            if (engine.setEntityModel(selectedEntity, path)) {
                unsavedChanges = true; log("Dropped: " + std::string(path));
            }
        }
...
void Editor::drawViewportContent() {
    ...
    char fps_str[48];
    snprintf(fps_str, sizeof(fps_str), "%d FPS  %ux%u", fps, viewportWidth, viewportHeight);
...
void Editor::drawConsoleContent() {
    ...
    auto filterBtn = [&](const char* label, int val, ImVec4 col) {
        if (consoleFilter == val) ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(col.x, col.y, col.z, 0.3f));
        else ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
        ImGui::PushStyleColor(ImGuiCol_Text, col);
        if (ImGui::SmallButton(label)) consoleFilter = val;
        ImGui::PopStyleColor(2);
        ImGui::SameLine();
    };
    ...
    if (ImGui::SmallButton("Clear")) consoleLogs.clear();
    ImGui::Separator();

    ImGui::BeginChild("##log", ImVec2(0, 0), ImGuiChildFlags_None, ImGuiWindowFlags_HorizontalScrollbar);
    for (const auto& e : consoleLogs) {
        if (consoleFilter >= 0 && (int)e.level != consoleFilter) continue;
        ImVec4 col; const char* pfx;
        switch (e.level) {
            case LogEntry::Warning: col = ImVec4(0.95f, 0.82f, 0.30f, 1.0f); pfx = "[W] "; break;
            case LogEntry::Error:   col = ImVec4(0.95f, 0.30f, 0.30f, 1.0f); pfx = "[E] "; break;
            default:                col = ImVec4(0.62f, 0.64f, 0.72f, 1.0f); pfx = "[I] "; break;
        }
...
void Editor::drawAssetBrowserContent() {
    ...
    if (ImGui::SmallButton("Up")) {
        ...
        if (!parent.empty()) { curDir = parent.string(); needsRefresh = true; }
    }
    ImGui::SameLine();
    if (ImGui::SmallButton("Refresh")) needsRefresh = true;
    ImGui::Separator();
    ...
    for (auto& entry : entries) {
        ...
        if (isDir)                                                     color = ImVec4(0.45f, 0.65f, 0.95f, 1.0f);
        else if (ext == ".zscene")                                     color = ImVec4(0.40f, 0.85f, 0.50f, 1.0f);
        else if (ext==".obj"||ext==".fbx"||ext==".glb"||ext==".gltf")  color = ImVec4(0.90f, 0.70f, 0.30f, 1.0f);
        else if (ext == ".png" || ext == ".jpg")                       color = ImVec4(0.75f, 0.50f, 0.80f, 1.0f);

        ImGui::TextColored(color, "%s %s", isDir ? ">" : " ", name.c_str());

        if (ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(0)) {
            if (isDir) { curDir = entry.path().string(); needsRefresh = true; }
            else if (ext == ".zscene") {
                ...
                } else log("Load failed!", LogEntry::Error);
            }
        }

        if (!isDir && (ext==".obj"||ext==".fbx"||ext==".glb"||ext==".gltf")) {
    ...
    if (ImGui::Begin(title, &showFileDialog)) {
        ...
        if (ref) {
            ...
            try {
                ...
                for (auto& e : std::filesystem::directory_iterator(dir)) de.push_back(e);
        ...
        if (ImGui::SmallButton("Up##fd")) {
            ...
            if (!p.empty()) { dir = p.string() + "/"; ref = true; }
        }
        ImGui::Separator();
        ...
        for (auto& e : de) {
            ...
            if (!d && x != ".zscene") continue;

            if (ImGui::Selectable((std::string(d ? "> " : "  ") + n).c_str(), false, ImGuiSelectableFlags_AllowDoubleClick)) {
                if (d) { dir = e.path().string() + "/"; ref = true; }
                else {
                    strncpy(fn, n.c_str(), sizeof(fn) - 1);
                    if (ImGui::IsMouseDoubleClicked(0) && !fileDialogSave) {
                        std::string fp = dir + fn;
                        if (engine.loadScene(fp)) {
                            currentScenePath = fp; selectedEntity = INVALID_ENTITY; unsavedChanges = false;
                            log("Loaded: " + fp);
                        } else log("Failed: " + fp, LogEntry::Error);
        ...
        if (ImGui::Button(fileDialogSave ? "Save" : "Open", ImVec2(52, 0))) {
            std::string fp = dir + fn;
            if (fp.find(".zscene") == std::string::npos) fp += ".zscene";
            if (fileDialogSave) {
                if (engine.saveScene(fp)) { currentScenePath = fp; unsavedChanges = false; log("Saved: " + fp); }
                else log("Save failed: " + fp, LogEntry::Error);
            } else {
                if (engine.loadScene(fp)) { currentScenePath = fp; selectedEntity = INVALID_ENTITY; unsavedChanges = false; log("Loaded: " + fp); }
                else log("Load failed: " + fp, LogEntry::Error);
            }
            showFileDialog = false;
        }
        ImGui::SameLine();
        if (ImGui::Button("Cancel", ImVec2(52, 0))) showFileDialog = false;
    }
    ImGui::End();
}
...
void Editor::drawSceneSettings() {
    ...
    if (ImGui::CollapsingHeader("Rendering", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        if (ImGui::Checkbox("Shadows", &shadows)) engine.setShadowsEnabled(shadows);
        if (ImGui::Checkbox("Skybox", &skybox)) engine.setSkyboxEnabled(skybox);
        if (ImGui::Checkbox("Post Processing", &pp)) engine.setPostProcessEnabled(pp);
    }
    if (ImGui::CollapsingHeader("Tone Mapping", ImGuiTreeNodeFlags_DefaultOpen)) {
        static float exposure = 1.0f, gamma = 2.2f;
        if (ImGui::SliderFloat("Exposure", &exposure, 0.1f, 5.0f)) engine.setExposure(exposure);
        if (ImGui::SliderFloat("Gamma", &gamma, 1.0f, 3.0f)) engine.setGamma(gamma);
    }
    if (ImGui::CollapsingHeader("Light", ImGuiTreeNodeFlags_DefaultOpen)) {
        static glm::vec3 dir(-0.5f, -1.0f, -0.3f), col(1, 0.98f, 0.9f);
        static float amb = 0.15f;
        bool ch = false;
        if (drawVec3("Direction", dir)) ch = true;
        if (ImGui::ColorEdit3("Color", &col.x)) ch = true;
        if (ImGui::SliderFloat("Ambient", &amb, 0, 1)) ch = true;
        if (ch) engine.setDirectionalLight(dir, col, amb);
    }
...
void Editor::drawStatusBar(float x, float y, float w, float h) {
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_WindowBorderSize, 0.0f);
    ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(0.098f, 0.102f, 0.122f, 1.0f));
    ...
    if (!currentScenePath.empty())
        ...
        ImGui::TextColored(ImVec4(0.45f, 0.47f, 0.55f, 1.0f), "Untitled%s", unsavedChanges ? " *" : "");
